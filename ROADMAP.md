...
- [ ] A **boxed tab strip**. Kaimon draws each tab as its own bordered box, the boxes joined into a strip, and the active one open at the bottom so it reads as continuous with the pane beneath. `TabStrip` paints a row of captions and reverses the active one; the joinery is the interesting part and there is no style knob for it. This is the largest remaining VISUAL difference between the rebuild and the original.
- [ ] A **pane of `RichText` lines**, scrollable and cursorless. `List` is a SELECTION widget, so a log pane borrows one whose whole point is choosing a row; `SelectMode.NONE` removes the semantics, not the shape. `MarkdownPane` is such a pane for one input format — a plain one would serve a log, a transcript and a diff.
